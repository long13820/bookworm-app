...
export default class Shop extends React.Component {
  ...
  async handlePageChange(pageNumber) {
    let finalFilter = `http://localhost:8000/api/filterby?paginate=${this.state.paginate}&page=${pageNumber}`;
  ...
  async getFilter() {
    ...
    await axios.get(url + 'categories').then((result) => {
      result.data.map((i) => {
    ...
    await axios.get(url + 'authors').then((result) => {
      result.data.map((i) => {
    ...
    if (name === 'Rating star') {
      if (item !== '') {
        await this.setState((prevState) => ({
          filter: {
            ...prevState.filter,
            rating: `&filter[avg_star]=${item.split(' ')[0]}`
          }
        }));
      } else {
        await this.setState((prevState) => ({
                  ...
                  <div className="row mb-4">
                    ...
                    <div className="col-lg-6 d-flex justify-content-end">
                      ...
                      <div>
                        ...
                        >
                                    <DropdownItem eventKey="paginate=5">
                                        Show: 5
                                    </DropdownItem>
                                    <DropdownItem eventKey="paginate=10">
                                        Show: 10
                                    </DropdownItem>
                                    <DropdownItem eventKey="paginate=15">
                                        Show: 15
                                    </DropdownItem>
                                    <DropdownItem eventKey="paginate=20">
                                        Show: 20
                                    </DropdownItem>
                        </Dropdown>
                      </div>
