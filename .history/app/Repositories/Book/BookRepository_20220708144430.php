...
//                    $filterKey2[] = $key;
            ...
            $allBooks =  DB::table('books')
                ...
                ->leftJoin('reviews', 'review.book_id', '=', 'books.id')
                ->leftJoin('discounts', 'discount.book_id', '=', 'books.id')
                ->selectRaw('avg(review.rating_star) as avg_rating')
                ...
                ->havingRaw('avg(review.rating_start) >= ?', array($filterValue2))
                ->select('books.id',
                    'books.book_title',
                    'books.book_price',
            ...
            $allBooks =  DB::table('books')
                ->join('authors', 'authors.id', '=', 'books.author_id')
                ->join('categories', 'category.id', '=', 'books.category_id')
                ->leftJoin('review', 'review.book_id', '=', 'books.id')
                ->leftJoin('discount', 'discount.book_id', '=', 'books.id')
                ->selectRaw('avg(review.rating_star) as avg_rating')
